...
- (void)viewDidLoad {
    ...
    [PerfectDebug logWithTag:@"Test->MyTag" log:@"viewDidLoad ... "];
    
    self.title = @"PerfectDebug Test";
    ...
    swcDebug.on = PerfectDebug.shared.isOn;
...
- (void)logOn:(UISwitch *)swc {
    PerfectDebug.shared.isOn = swc.on;
    [self.textView resignFirstResponder];
}

- (void)makeLog {
    [PerfectDebug logWithTag:@"Test->TestTag" log:@"SEL name : %@", NSStringFromSelector(_cmd)];
    [PerfectDebug logWithTag:@"Test->tag1" content:@{@"pk":@"pv"}];
    [PerfectDebug log:@"我是一条无Tag的log"];
}
...
- (void)sessionDataTaskRequest {
    ...
    NSURLSessionDataTask *task = [session dataTaskWithRequest:request completionHandler:^(NSData * _Nullable data, NSURLResponse * _Nullable response, NSError * _Nullable error) {
        [PerfectDebug logWithTag:@"Test->NSURLSession" log:@"netRequestURLSession : %@ , error : %@", [[NSString alloc] initWithData:data encoding:NSUTF8StringEncoding], error];
    }];
    [task resume];
}
...
- (void)sessionDownloadTaskRequest {
    ...
    NSURLSessionDownloadTask *task = [session downloadTaskWithRequest:request completionHandler:^(NSURL * _Nullable location, NSURLResponse * _Nullable response, NSError * _Nullable error) {
        [PerfectDebug logWithTag:@"Test->NSURLSession" log:@"download task不会被拦截"];
    }];
    [task resume];
}
...
- (void)afnRequest {
    ...
    [manager GET:@"https://getman.cn/echo" parameters:@{@"afnRequest":@"v1",@"p2":@"v2"} headers:nil progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
            [PerfectDebug logWithTag:@"Test->AFRequest" log:@"responseObject : %@", responseObject];
        } failure:^(NSURLSessionDataTask * _Nullable task, NSError * _Nonnull error) {
            [PerfectDebug logWithTag:@"Test->AFRequest" log:@"error : %@", error];
        }];
}
...
//    [PerfectDebug logConsole:@"%@", [DebugBackTrace dg_backtraceOfCurrentThread]];
//    [PerfectDebug logConsole:@"%@", [NSThread callStackSymbols]];
//    float a = 0;
//    float b = 0.0f;
//    printf("%f", a/b);
//    abort();
...
- (void)signalCrashWithErrorAddress:(id)sender{
    //    char *s = "hello world";
    //    [PerfectDebug logWithTag:@"Test->signalCrashWithErrorAddress" log:@"%p %p", &s, s, *s];
    //    *s = 'H';
}
