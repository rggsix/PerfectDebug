//
//  DebugLogManager+NetworkMonitor.m
//  PerfectDebug
//
...
@implementation DebugLogManager (NetworkMonitor)
...
- (void)recordNetRequestWithRequest:(NSURLRequest *)request content:(id)content response:(id)response error:(nullable NSError *)error{
    ...
    if (DebugNetworkMonitor.shared.responseJudgeBlock) {
        isErrorResponse = DebugNetworkMonitor.shared.responseJudgeBlock(request, responseDict) == NO;
    }
    ...
    [DebugLogManager.shared recordLogWithTag:kDebugNetworkLogTag content:rcontent complete:nil];
}
