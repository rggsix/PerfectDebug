...
#import "DebugSessionManager.h"
#import "DebugLogManager+NetworkMonitor.h"
#import "DebugNetworkMonitor.h"


static NSString * _Nonnull const DebugURLProtocolHandledKey = @"DebugURLProtocolHandledKey";

@interface DebugHTTPProtocol ()<NSURLSessionTaskDelegate>
...
+ (BOOL)canInitWithRequest:(NSURLRequest *)request{
    if (!PerfectDebug.shared.isOn) return NO;
...
+ (NSURLRequest *)canonicalRequestForRequest:(NSURLRequest *)request {
    if (!PerfectDebug.shared.isOn) return request;
...
- (void)startLoading {
    ...
    self.task = [DebugSessionManager.sharedManager dataTaskWithRequest:mrequest delegate:self];
    [self.task resume];
}
...
- (void)URLSession:(NSURLSession *)session task:(NSURLSessionTask *)task didCompleteWithError:(NSError *)error {
    ...
    BOOL isIgnore = false;
    NSArray *ignoreList = [DebugNetworkMonitor.shared.ignoreUrlList copy];
    ...
    if(!isIgnore){
        [DebugLogManager.shared recordNetRequestWithRequest:curRequest
                                                    content:content
    ...
    if (notGET && emptyBody) {
        ...
        while (!end) {
            ...
            if (!bytes) { // end
                ...
                [PerfectDebug logConsole:@"[PerfectDebug] %@", msg];
                end = YES;
