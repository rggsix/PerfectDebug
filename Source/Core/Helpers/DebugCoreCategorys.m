...
//  PerfectDebug
//
//  Created by perfectword on 2020/12/9.
//
...
@implementation NSObject (PerfectDebug)
...
@implementation NSDate (PerfectDebug)
...
@implementation UIViewController (PerfectDebug)

+ (instancetype)dg_currentController{
    UIViewController *rootVC = [UIApplication sharedApplication].keyWindow.rootViewController;
    if (!rootVC) {
        rootVC = [DebugUtil currentWindow].rootViewController;
    }
    if (!rootVC) {
        return nil;
    }
...
+ (void)dg_presentController:(UIViewController *)controller
    ...
    if (!presentVC) {
        [PerfectDebug logConsole:@"[PerfectDebug] Can't find useable view controller to present DebugStartupController!"];
        return;
    }
...
@implementation UIView (PerfectDebug_frame)
