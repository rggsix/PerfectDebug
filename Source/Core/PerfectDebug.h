...
#ifndef PerfectDebug_h
#define PerfectDebug_h

#import <Foundation/Foundation.h>

typedef NS_ENUM(NSUInteger, PerfectDebugModule) {
    ///  网络监控
    PerfectDebugNetMonitor = 1 << 0,
    ///  性能监控
    PerfectDebugPerformance = 1 << 1,
};
...
/// 开关通知
static NSString * _Nonnull const PerfectDebugIsOnChangedNotificationName = @"PerfectDebugIsOnChanged";
...
@interface PerfectDebug : NSObject

///  允许的 Tag, 比如你发布到线上, 只想记录crash, 设置为 `PerfectDebug.shared.validTags = @[kDebugCrashLogTag]` 即可
@property(nonatomic, strong, nullable, class) NSArray<NSString *> *validTags;

+ (instancetype)shared;
///  启动(配置) PerfectDebug, 主模块是必须配置的, 其他的自选, 见 ``PerfectDebugModule``
...
///  生成log缩略内容回调，PerfectDebug内置生成逻辑(比如Network log就是URL, 详见DebugLogContentModel)。 此回调返回字符串将替换内置生成逻辑
