...
@implementation DebugDisplayer

+ (instancetype)shared {
    static DebugDisplayer *displayer;
...
- (void)beginAddWindowLoop {
    self.entrySetupTimer = [NSTimer scheduledTimerWithTimeInterval:5 repeats:YES block:^(NSTimer * _Nonnull timer) {
        ...
        UIWindow *window = [DebugUtil currentWindow];
...
- (void)setupBaseUIIfNeed{
    ...
    [self.consoleEntryBtn setImage:[DebugUtil imageNamed:@"debug_entry_icon"] forState:UIControlStateNormal];
...
- (void)showStartupListController{
    DebugStartupListController *displayController = [DebugStartupListController new];
